...
class dCleaner:
    ...
    def __init__(self, options):
        ...
        if self.options_.folder_ == "\\" or (
                path.exists(self.options_.folder_) and not path.isdir(self.options_.folder_)):
    ...
    def __repr__(self):
        ...
        #
        val = self.paddingFolder_.partitionUsage()

        if self.options_.verbose_:
            out = "Paramètres : "
            out += "\n\t- Mode : " + self.options_.color_.colored("nettoyage" if self.options_.clear_ else (
                "ajustement" if self.options_.adjust_ else "remplissage / nettoyage"), formatAttr=[textAttribute.GRAS])
            out += "\n\t- Taux de remplissage max : " + self.options_.color_.colored(str(self.options_.fillRate_) + "%",
                                                                                     formatAttr=[textAttribute.GRAS])
            out += "\n\t- Taux de renouvellement de la partition : " + self.options_.color_.colored(
                str(self.options_.renewRate_) + "%", formatAttr=[textAttribute.GRAS])

            if not self.options_.adjust_:
                out += "\n\t- Itération(s) de nettoyage : " + self.options_.color_.colored(str(self.options_.iterate_),
                                                                                           formatAttr=[
                                                                                               textAttribute.GRAS])
            ...
            out += "\n\nPartition : "
            out += "\n\t- Taille : " + self.paddingFolder_.displaySize(val[0])
            out += "\n\t- Remplie à "
            out += self.options_.color_.colored(str(round(val[1]/val[0]*100, 2))+"%", formatAttr=[textAttribute.GRAS])
            out += " - " + self.paddingFolder_.displaySize(val[1])

            out += "\n\nDossier : "
            out += "\n\t- Nom : " + self.options_.color_.colored(self.paddingFolder_.name(),
                                                                 formatAttr=[textAttribute.GRAS])
            out += "\n\t- Contenu : " + self.paddingFolder_.displaySize(self.paddingFolder_.size()) + "\n"
        else:
            out = "Partition : " + self.paddingFolder_.displaySize(val[0]) + " - remplie à " + str(
                round(val[1] / val[0] * 100, 0)) + "%"
            out += "\nDossier : " + self.options_.color_.colored(self.paddingFolder_.name(),
                                                                 formatAttr=[textAttribute.GRAS])
            out += "\nMode : " + ("nettoyage" if self.options_.clear_ else (
                "ajustement" if self.options_.adjust_ else "remplissage / nettoyage"))
            out += "\nTaux de remplissage max : " + self.options_.color_.colored(str(self.options_.fillRate_) + "%",
                                                                                 formatAttr=[textAttribute.GRAS])
            out += "\nTaux de renouvellement de la partition : " + self.options_.color_.colored(
                str(self.options_.renewRate_) + "%", formatAttr=[textAttribute.GRAS])

            if self.options_.adjust_ is False:
                out += "\nItération(s) de nettoyage : " + self.options_.color_.colored(str(self.options_.iterate_),
                                                                                       formatAttr=[textAttribute.GRAS])
    ...
    def fillPartition(self):
        ...
        val = self.paddingFolder_.partitionUsage()
        maxFill = val[0] * self.options_.fillRate_ / 100

        if val[1] < maxFill:
            # On fait en sorte de coller immédiatement au taux de remplissage
            fillSize = maxFill - val[1]
    ...
    def freePartition(self):
        ...
        if currentFillSize > maxFillSize:
            if self.options_.verbose_:
                print(self.options_.color_.colored(
                    "La partition est déja trop remplie (" + self.paddingFolder_.displaySize(
                        currentFillSize) + " - " + str(round(currentFillSize / totalSize * 100, 0)) + "% )",
                    textColor.JAUNE))
            ...
            if gap > paddingFillSize:
                ...
                print(self.options_.color_.colored(
                    "Le vidage du dossier de remplissage ne sera pas suffisant pour atteindre le taux de remplissage "
                    "demandé",
                    textColor.JAUNE))
                result = self.paddingFolder_.empty()
                print(self.options_.color_.colored("Dossier de 'padding' vidé", formatAttr=[textAttribute.GRAS]))

                if len(result[1]) > 0:
                    print(self.options_.color_.colored("Erreur lors du vidage du dossier de remplissage : " + result[1],
                                                       textColor.ROUGE))
                    return False
            else:
                # Retrait du "minimum"
    ...
    def cleanPartition(self):
        ...
        renewSize = val[0] * (100 - self.options_.fillRate_) / 100 * self.options_.renewRate_ / 100

        # on recadre avec l'espace effectivement dispo
        renewSize = parser.cmdLineParser.minMax(None, 0, renewSize, val[2] * self.options_.renewRate_ / 100)
...
if '__main__' == __name__:
    ...
    # La fin, la vraie !
    print(params.color_.colored("Fin des traitements", datePrefix=(not params.verbose_)))
